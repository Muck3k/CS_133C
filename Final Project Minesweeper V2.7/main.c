...
int main(int argc, char *argv[])
{
  ...
  FILE *description;
  //File write:
  game_history = fopen("game_history.txt", "w");
  fprintf(game_history, " This is the game history: \n");
...
void print_board(){
  ...
  for(int row = 0; row < BOARD_SIZE; ++row)
    ...
    if(row % 4 == 0 && row / 4 == 1){
      ...
      fprintf(game_history, " ");
      fputc(board[row][col], game_history);
		}
	}
  printf("\n\n");
  fprintf(game_history, "\n\n");
  ...
  for(int row = 0; row < BOARD_SIZE; ++row)
    ...
    if(row % 4 == 0 && row / 4 == 1){
      ...
      if(mine_board[row][col] == '*'){
        ...
        fprintf(game_history, " ");
        fputc(mine_board[row][col], game_history);
      }else{
        printf("%c  ", board[row][col]);
        fprintf(game_history, " ");
        fputc(board[row][col], game_history);
